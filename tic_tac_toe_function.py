...
def instructions():
    ...
    while True:
        ...
        if len(computer_symbol) == 1 and computer_symbol != human_symbol:
            break
...
def hard_computers_turn(table):
    ...
    temp_dict = {
        number: state
        for (number, state) in game.items()
        if type(number) == int
    }
    if game["turn"] >= 4:
        for i in temp_dict:
    ...
    if game["turn"] >= 3:
        for [a, b, c] in WINNING_POSITIONS:
            if (
                game[a] == game[b] == game["human"]["symbol"]
                and game[c] == "No"
            ):
                j = c
                break
            elif (
                game[b] == game[c] == game["human"]["symbol"]
                and game[a] == "No"
            ):
                j = a
                break
            elif (
                game[a] == game[c] == game["human"]["symbol"]
                and game[b] == "No"
            ):
                j = b
                break
